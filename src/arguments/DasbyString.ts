...
export class DasbyArgument extends Argument<string> {
	...
	public async run(argument: string, context: ArgumentContext) {
		if (typeof context.minimum === 'number' && argument.length < context.minimum) {
			return this.error(argument, 'ArgumentStringTooShort', await context.message.fetchLanguageKey('arguments/string:argumentStringTooShort'));
		}
		if (typeof context.maximum === 'number' && argument.length > context.maximum) {
			return this.error(argument, 'ArgumentStringTooLong', await context.message.fetchLanguageKey('arguments/string:argumentStringTooLong'));
		}
